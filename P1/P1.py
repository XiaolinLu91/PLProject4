...
s_dept = (('ID','NAME', 'REGION_ID'),
        (10, 'FINANCE', 1),
        (31, 'SALES', 1), (32, 'SALES', 2),
        (33, 'SALES', 3),(34,'SALES',4),(35,'SALES',5),(41,'OPERATIONS',1),(42,'OPERATIONS',2), (43,'OPERATIONS',3),(44,'OPERATIONS',4),(45,'OPERATIONS',5),(50,'ADMINISTRATION',1))
...
i = []
for dept_id in { d[9] for d in s_emp[1::] }: i.append([dept_id, (lambda l: round(sum(l) / len(l), 2))(map(float,[ e[7] for e in s_emp[1::] if e[9] == dept_id ]))])
print sorted(i,key = lambda x:int(x[0]))

# 8
print "\nselect dept_id, avg(salary) from s_emp group by dept_id having avg(salary) < 1500: "
i = []
for dept_id in { d[9] for d in s_emp[1::] }:
    avg_sal = (lambda l: round(sum(l) / len(l), 2))(map(float,[ e[7] for e in s_emp[1::] if e[9] == dept_id ]))
    if avg_sal < 1500: i.append([dept_id, avg_sal])
print i
